...
node *createNode(int id, int course, int grade) {
...
node *readTable(FILE *f, node *head) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        // Parse the line
        if (sscanf(line, "%d,%d,%d", &id, &course, &grade) != 3) {
            continue;
        }

        // Create a new node
        node *newNode = createNode(id, course, grade);
        if (newNode == NULL) {
            return NULL;
        }
...
void runQuery(char *query, char *schema, const char *filename) {
    ...
    if (strstr(lowerQuery, "select") != NULL) {
        ...
        // Select data
        node *head = NULL;
        head = selectData(query, schema, head, f);

        // Print the results
        printResults(head, (const char *[]){"id", "course", "grade"}, 3);
        ...
        // Close the file
        fclose(f);
    } else if (strstr(lowerQuery, "insert") != NULL) {
        // Open the file
        ...
        // Insert data
        insertData(query, schema, f);

        // Close the file
        fclose(f);
    } else if (strstr(lowerQuery, "delete") != NULL) {
        // Open the file
        FILE *f = fopen(filename, "r+");
        if (f == NULL) {
            return;
        ...
        // Delete data
        node *head = NULL;
        head = deleteData(query, schema, head, f);
    ...
    if (whereClause != NULL) {
        while (current != NULL) {
            ...
            for (int i = 0; i < numSelectColumns; i++) {
                ...
                } else if (strcmp(selectColumns[i], "grade") == 0) {
